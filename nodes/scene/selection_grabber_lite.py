...
class SvSelectionGrabberLite(bpy.types.Node, SverchCustomTreeNode):
    ...
    selected_mode = bpy.props.EnumProperty(
        items=[(k, k, '', i) for i, k in enumerate(["Live", "Freeze"])], 
        description="offers , yes what does it offer",
        default="Live", update=updateNode
    )
    ...
    def get_frozen_states(self):
        return self.node_dict.get(hash(self), ([], [], []))

    def store_states_in_dict(self):
    ...
    def process(self):
        if self.selected_mode == "Live":
            ...
            vertex_mask, edge_mask, face_mask = self.get_frozen_states()
